...
import ru.samtakoy.oldlegacy.core.presentation.qpack.list.QPacksListAdapter.ItemClickListener
...
import ru.samtakoy.oldlegacy.core.presentation.qpack.list.vm.QPacksListViewModel
...
open class QPacksListFragment : Fragment(), ViewModelOwner {
    ...
    private val mTextListener: SearchView.OnQueryTextListener = object : SearchView.OnQueryTextListener {
        override fun onQueryTextSubmit(query: String): Boolean {
            viewModel.onEvent(QPacksListViewModel.Event.SearchTextChange(query))
            return false
        }

        override fun onQueryTextChange(newText: String): Boolean {
            viewModel.onEvent(QPacksListViewModel.Event.SearchTextChange(newText))
            return false
        }
    }
    ...
    private val mCheckBoxListener =
        CompoundButton.OnCheckedChangeListener { compoundButton: CompoundButton?, b: Boolean ->
            viewModel.onEvent(QPacksListViewModel.Event.FavoritesCheckBoxChange(b))
        }

    private val mTabSelectedListener: OnTabSelectedListener = object : OnTabSelectedListener {
        override fun onTabSelected(tab: TabLayout.Tab?) {
            if (tab === mTabByLastViewDate) {
                viewModel.onEvent(QPacksListViewModel.Event.SortByLastViewDate)
            } else if (tab === mTabByCreationDate) {
                viewModel.onEvent(QPacksListViewModel.Event.SortByCreationDate)
            }
        }
        ...
        mAdapter = QPacksListAdapter(
            object: ItemClickListener {
                override fun onClick(item: QPackListItemUiModel) {
                    viewModel.onEvent(QPacksListViewModel.Event.PackClick(item))
                }
            }
        )
    ...
    private fun onAction(action: QPacksListViewModel.Action) {
        when (action) {
            is QPacksListViewModel.Action.ShowErrorMessage -> showError(action.message)
            is QPacksListViewModel.NavAction.ShowPackInfo -> showPackInfo(action.qPackId)
            is QPacksListViewModel.Action.SearchText -> setSearchText(action.text)
        }
    }

    private fun onViewState(state: QPacksListViewModel.State) {
