package ru.samtakoy.oldlegacy.core.presentation.courses.list
...
import ru.samtakoy.oldlegacy.core.presentation.courses.CourseEditDialogFragment
import ru.samtakoy.oldlegacy.core.presentation.courses.list.vm.CoursesListViewModel
import ru.samtakoy.oldlegacy.core.presentation.courses.list.vm.CoursesListViewModelImpl
import ru.samtakoy.oldlegacy.core.presentation.courses.model.CourseItemUiModel
import ru.samtakoy.oldlegacy.core.presentation.courses.model.CoursesAdapter
import ru.samtakoy.oldlegacy.core.presentation.export_cards.BatchExportDialogFragment
import ru.samtakoy.oldlegacy.core.presentation.showDialogFragment
...
class CoursesListFragment : Fragment(), CoursesAdapter.CourseClickListener, ViewModelOwner {
    ...
    private val menuProvider = object : MenuProvider {
        ...
        override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
            when (menuItem.getItemId()) {
                R.id.menu_item_add -> {
                    viewModel.onEvent(CoursesListViewModel.Event.AddCourseRequestClick)
                    return true
                }
                R.id.menu_item_send_courses -> {
                    viewModel.onEvent(CoursesListViewModel.Event.BatchExportToEmailClick)
                    return true
                }
            }
            return false
    ...
    private fun onAction(action: CoursesListViewModel.Action) {
        when (action) {
            is CoursesListViewModel.Action.ShowAddCourseDialog -> showAddCourseDialog(action.defaultTitle)
            CoursesListViewModel.Action.ShowBatchExportToEmailDialog -> showBatchExportToEmailDialog()
            is CoursesListViewModel.Action.ShowErrorMessage -> showError(action.message)
            is CoursesListViewModel.NavigationAction.NavigateToCourseInfo -> navigateToCourseInfo(action.courseId)
        }
    }

    private fun onViewState(state: CoursesListViewModel.State) {
    ...
    private fun showBatchExportToEmailDialog() {
        val dialog = BatchExportDialogFragment.Companion.newCoursesFragment(
            targetFragment = this,
    ...
    private fun showAddCourseDialog(defaultTitle: String) {
        val dialog = CourseEditDialogFragment.Companion.newDialog(defaultTitle)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        when (requestCode) {
            REQ_CODE_ADD_COURSE -> {
                val courseTitle = data?.getStringExtra(CourseEditDialogFragment.Companion.RESULT_EXTRA_TEXT)
                viewModel.onEvent(CoursesListViewModel.Event.AddNewCourseConfirm(courseTitle))
            }
        }
    }
    ...
    override fun onCourseClick(course: CourseItemUiModel) {
        viewModel.onEvent(CoursesListViewModel.Event.CourseClick(course))
    }
