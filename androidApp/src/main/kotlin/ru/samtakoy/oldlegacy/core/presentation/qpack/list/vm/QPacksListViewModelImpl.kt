package ru.samtakoy.oldlegacy.core.presentation.qpack.list.vm
...
internal class QPacksListViewModelImpl(
...
) : BaseViewModelImpl<QPacksListViewModel.State, QPacksListViewModel.Action, QPacksListViewModel.Event>(
    scopeProvider = scopeProvider,
    initialState = QPacksListViewModel.State(
    ...
    init {
        subscribeData()
        launchCatching {
            sendAction(QPacksListViewModel.Action.SearchText(searchText.value))
        }
    }

    override fun onEvent(event: QPacksListViewModel.Event) {
        when (event) {
            is QPacksListViewModel.Event.FavoritesCheckBoxChange -> onUiFavoritesCheckBoxChange(event.isChecked)
            is QPacksListViewModel.Event.PackClick -> onUiPackClick(event.item)
            is QPacksListViewModel.Event.SearchTextChange -> onUiSearchTextChange(event.text)
            QPacksListViewModel.Event.SortByCreationDate -> onUiSortByCreationDate()
            QPacksListViewModel.Event.SortByLastViewDate -> onUiSortByLastViewDate()
        }
    }
    ...
    private fun onUiPackClick(qPack: QPackListItemUiModel) {
        sendAction(QPacksListViewModel.NavAction.ShowPackInfo(qPack.id.value))
    }
    ...
    private fun onGetError(t: Throwable) {
        ...
        sendAction(
            QPacksListViewModel.Action.ShowErrorMessage(resources.getString(R.string.qpacks_request_error))
        )
    }
