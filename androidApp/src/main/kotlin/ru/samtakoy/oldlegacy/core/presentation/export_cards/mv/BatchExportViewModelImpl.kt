...
import ru.samtakoy.oldlegacy.core.presentation.export_cards.BatchExportType
...
internal class BatchExportViewModelImpl(
...
) : BaseViewModelImpl<Unit, BatchExportViewModel.Action, Unit>(
    scopeProvider = scopeProvider,
    initialState = Unit
    ...
    private fun onGetError(t: Throwable) {
        ...
        sendAction(
            BatchExportViewModel.Action.ExitWithError(resources.getString(R.string.fragment_dialog_batch_export_error_msg))
        )
    }

    private suspend fun exportCoursesToEmail(exporter: CoursesExporter) {
        exporter.exportAllToEmail()
        sendAction(BatchExportViewModel.Action.ExitOk)
    }
